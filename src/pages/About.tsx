"use client";

import DecorativeCircle from "@/components/DecorativeCircle";
import { motion } from "framer-motion";
import { ArrowRight, Quote, Users, Target, Shield, Handshake } from "lucide-react";

const teamBanner = "/assets/team/team-banner.jpg";
const harrietImg = "/assets/team/harriet-forrest.webp";
const markImg = "/assets/team/mark-taylor.webp";

const BOOKING_URL = "https://meetings.hubspot.com/fortayconnect";

const fadeUp = {
  hidden: { opacity: 0, y: 24 },
  visible: { opacity: 1, y: 0 }
};

const values = [
  {
    icon: Target,
    title: "Outcomes over opinions",
    description:
      "Every recommendation is anchored to a measurable business outcome, cost reduction, efficiency gain, or revenue impact."
  },
  {
    icon: Shield,
    title: "Platform-neutral, always",
    description:
      "We have no single-platform allegiance. We evaluate every platform against your specific needs, not our commercial interests."
  },
  {
    icon: Users,
    title: "In-house expertise",
    description:
      "Our team has architected, deployed, and managed comms platforms at Vodafone, Google, AWS, and Mitel. We know what works at scale."
  },
  {
    icon: Handshake,
    title: "Partnership, not projects",
    description:
      "Our average client relationship lasts six years. We measure success at 18 months post-deployment, not on go-live day."
  },
];

const stats = [
  { value: "200+", label: "Enterprise transformations delivered" },
  { value: "6+ yrs", label: "Average client retention" },
  { value: "\u00a3Millions", label: "In savings identified for clients" },
  { value: "15+", label: "Technology partnerships" },
];

const About = () => (
  <>
    <main>
      {/* HERO BANNER */}
      <section className="relative overflow-hidden">
        <div className="absolute inset-0">
          <img
            src={teamBanner}
            alt="The Fortay Connect team at a technology conference"
            className="h-full w-full object-cover"
          />
          <div className="absolute inset-0 bg-gradient-to-b from-foreground/80 via-foreground/70 to-foreground/90" />
        </div>
        <div className="container relative py-28 md:py-40">
          <motion.div
            className="mx-auto max-w-3xl text-center"
            initial="hidden"
            animate="visible"
            variants={{ visible: { transition: { staggerChildren: 0.12 } } }}
          >
            <motion.p
              variants={fadeUp}
              className="mb-4 inline-block rounded-full border border-primary/30 bg-primary/10 px-4 py-1.5 text-sm font-semibold uppercase tracking-widest text-primary"
            >
              About Fortay Connect
            </motion.p>
            <motion.h1
              variants={fadeUp}
              className="font-display text-4xl font-extrabold leading-[1.1] tracking-tight text-background md:text-5xl lg:text-6xl"
            >
              A letter from our founders
            </motion.h1>
            <motion.p
              variants={fadeUp}
              className="mx-auto mt-5 max-w-xl text-lg text-background/60"
            >
              Why we built Fortay Connect, and why it matters for your business.
            </motion.p>
          </motion.div>
        </div>
      </section>

      {/* FOUNDER LETTER */}
      <section className="relative overflow-hidden py-20 md:py-28">
        <DecorativeCircle color="teal" className="-right-32 -top-32 opacity-10" />
        <div className="container relative">
          <motion.div
            className="mx-auto max-w-3xl"
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true }}
            variants={{ visible: { transition: { staggerChildren: 0.1 } } }}
          >
            <motion.div variants={fadeUp} className="mb-8 flex justify-center">
              <Quote className="h-10 w-10 text-primary/30" />
            </motion.div>

            <motion.h2
              variants={fadeUp}
              className="font-display text-3xl font-bold text-foreground md:text-4xl"
            >
              Enterprise communication has changed.
            </motion.h2>

            <motion.div variants={fadeUp} className="mt-8 space-y-5 text-lg leading-relaxed text-muted-foreground">
              <p>
                Most enterprise transformation initiatives fail. Not because the technology is wrong,
                but because the advice is conflicted. We built <strong className="text-foreground">Fortay Connect</strong> to fix that.
              </p>
              <p>
                Harriet led cloud transformation programmes at Google and AWS, working directly with
                enterprise clients navigating communication stack overhauls. Mark devoted 15 years to
                architecting and deploying communications solutions at Vodafone and Mitel. The pattern
                was the same everywhere: suppliers promising outcomes they had no accountability to deliver.
              </p>
              <p>
                We built Fortay Connect in 2019 because we could not find what our clients needed: a
                genuinely independent consultancy that stayed accountable from strategy through to
                commercial outcome. Our name comes from combining our surnames (Forrest + Taylor), but
                it represents the commitment that defines every engagement.
              </p>
              <p>
                Since then, we have helped <strong className="text-foreground">200+ organisations</strong> transform
                how they connect. From enabling global law firms to deliver borderless service to helping
                healthcare recruiters cut response times during critical shortages. CA Auto Finance came
                to us with three disconnected platforms. Six weeks later: one platform, costs down 40%,
                and a plan the board approved without challenge.
              </p>
              <p className="text-foreground font-medium">
                If you are facing a transformation that has to succeed, we would welcome the conversation.
                We have been in your position. We know what works.
              </p>
            </motion.div>

            {/* Founder signatures */}
            <motion.div
              variants={fadeUp}
              className="mt-12 flex flex-col items-center gap-8 sm:flex-row sm:justify-center"
            >
              <div className="flex items-center gap-4">
                <img
                  src={harrietImg}
                  alt="Harriet Forrest"
                  className="h-16 w-16 rounded-full border-2 border-primary/20 object-cover"
                />
                <div>
                  <p className="font-display font-bold text-foreground">Harriet Forrest</p>
                  <p className="text-sm text-muted-foreground">Co-founder</p>
                </div>
              </div>
              <div className="hidden h-8 w-px bg-border sm:block" />
              <div className="flex items-center gap-4">
                <img
                  src={markImg}
                  alt="Mark Taylor"
                  className="h-16 w-16 rounded-full border-2 border-primary/20 object-cover"
                />
                <div>
                  <p className="font-display font-bold text-foreground">Mark Taylor</p>
                  <p className="text-sm text-muted-foreground">Co-founder</p>
                </div>
              </div>
            </motion.div>
          </motion.div>
        </div>
      </section>

      {/* STATS BAR */}
      <section className="bg-foreground py-20 md:py-28">
        <div className="container">
          <div className="grid grid-cols-2 gap-8 md:grid-cols-4">
            {stats.map((stat, i) => (
              <motion.div
                key={stat.label}
                className="text-center"
                initial={{ opacity: 0, y: 16 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: i * 0.1 }}
              >
                <p className="font-display text-3xl font-bold text-primary md:text-4xl">
                  {stat.value}
                </p>
                <p className="mt-2 text-sm text-background/50">{stat.label}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* OUR VALUES */}
      <section className="relative overflow-hidden py-20 md:py-28">
        <DecorativeCircle color="purple" className="-left-40 bottom-0 opacity-10" />
        <div className="container relative">
          <motion.div
            className="mx-auto mb-14 max-w-2xl text-center"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          >
            <p className="mb-2 inline-block rounded-full border border-primary/30 bg-primary/10 px-4 py-1.5 text-sm font-semibold uppercase tracking-widest text-primary">
              Our approach
            </p>
            <h2 className="mt-4 font-display text-3xl font-bold text-foreground md:text-4xl">
              Why businesses choose Fortay Connect
            </h2>
            <p className="mt-4 text-lg text-muted-foreground">
              We are not a reseller. We are not a technology supplier. We are independent consultants who exist to get you the right result.
            </p>
          </motion.div>

          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {values.map((value, i) => (
              <motion.div
                key={value.title}
                className="rounded-2xl border border-border bg-card p-8 transition-shadow hover:shadow-lg"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: i * 0.08 }}
              >
                <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10">
                  <value.icon className="h-6 w-6 text-primary" strokeWidth={1.5} />
                </div>
                <h3 className="font-display text-lg font-bold text-foreground">{value.title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
                  {value.description}
                </p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* MEET THE FOUNDERS */}
      <section className="bg-foreground py-20 md:py-28">
        <div className="container">
          <motion.div
            className="mx-auto max-w-3xl text-center"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          >
            <p className="mb-3 inline-block rounded-full border border-primary/30 bg-primary/10 px-4 py-1.5 text-sm font-semibold uppercase tracking-widest text-primary">
              Meet the founders
            </p>
            <h2 className="font-display text-3xl font-bold text-background md:text-4xl">
              Talk to the people who do the work
            </h2>
            <p className="mx-auto mt-4 max-w-xl text-lg text-background/60">
              No account managers. No sales team. Book a 30-minute call with Harriet or Mark and get a straight assessment of where you stand and what is worth doing next.
            </p>

            <div className="mt-10 grid gap-6 sm:grid-cols-2">
              {[
                {
                  img: harrietImg,
                  name: "Harriet Forrest",
                  focus: "Cloud transformation, programme governance, commercial strategy. Previously Google and AWS.",
                },
                {
                  img: markImg,
                  name: "Mark Taylor",
                  focus: "Communications architecture, platform selection, deployment leadership. Previously Vodafone and Mitel.",
                },
              ].map((founder) => (
                <div
                  key={founder.name}
                  className="flex flex-col items-center rounded-2xl border border-background/10 bg-background/5 px-6 py-8"
                >
                  <img
                    src={founder.img}
                    alt={founder.name}
                    className="h-20 w-20 rounded-full border-4 border-primary/30 object-cover"
                  />
                  <p className="mt-4 font-display text-lg font-bold text-background">
                    {founder.name}
                  </p>
                  <p className="mt-2 text-sm leading-relaxed text-background/60">
                    {founder.focus}
                  </p>
                </div>
              ))}
            </div>

            <div className="mt-8">
              <a
                href={BOOKING_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 rounded-full bg-primary px-8 py-3.5 text-base font-bold text-primary-foreground shadow-lg shadow-primary/30 transition-all hover:brightness-110"
              >
                Book Your Free Audit
                <ArrowRight className="h-4 w-4" />
              </a>
            </div>
          </motion.div>
        </div>
      </section>
    </main>
  </>
);

export default About;
